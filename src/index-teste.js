...
const LIMITE_RECONHECIMENTO = 0.55; // Distância máxima para considerar um rosto como correspondente
...
(async () => {
  ...
  chrome.runtime.sendMessage({ tipo: "GET_ALL_ALUNOS_DATA" }, async (resposta) => {
    ...
    const labeledDescriptors = listaDeAlunos.map(aluno => {
        const descriptors = aluno.vetoresFaciais.map(v => new Float32Array(Object.values(v)));
        return new faceapi.LabeledFaceDescriptors(aluno.nome, descriptors);
    });
